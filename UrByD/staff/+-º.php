...
$sql = "SELECT p.*
		,c.category_name 
		,un.unit_name
		,a.order_detail_qty ,a.order_detail_price
		,ord.ord_name
        FROM aby_product p
        INNER JOIN aby_category c on c.category_id = p.category_id
		INNER JOIN aby_unit un on un.unit_id = p.unit_id 
		INNER JOIN aby_order_detail a on a.product_id = p.product_id
		WHERE a.order_id = 'OR0001'  ";
...
$MYsql = "SELECT * FROM aby_order WHERE order_id = 'OR0001' ";
      ...
      <div class="row">
        <div class="col">
			...
			<div class="card-header border-0">
              <div class="row align-items-center">
                <div class="col">
                  <h3 class="mb-0">พนักงาน </h3><?php echo $staff_name;?>
                </div>
              </div>
            </div>
            ...
            <div class="card-header border-0">
              <div class="row ">
                <div class="col-lg-6">
                  <h3 class="mb-0">โต๊ะ </h3><?php echo $MYresult["order_seat"] ?>
                </div>
                <div class="col-lg-6">
                  <h3 class="mb-0">สถาณะชำระเงิน </h3><?php echo $MYresult["ord_name"] ?> 
                </div>
              </div>
            </div>
            ...
            <div class="card-header border-0">
              <div class="row align-items-center">
                <div class="col">
                  <h3 class="mb-0">วันที่ขายสินค้า </h3><?php echo $MYresult["order_createddate"] ?> 
                </div>
              </div>
            </div>
              ...
              <table class="table align-items-center table-flush">
                ...
                ?>
                  <tr>
                    ...
                    <td class="budget">
                        <?php echo $result["order_seat"];?>
                    </td>
                    <td class="budget">
                        <?php echo $result["ord_name"];?>
                    </td>
